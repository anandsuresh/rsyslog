...
describe 'rsyslog::default' do
  ...
  context '/etc/rsyslog.d directory' do
    ...
    context 'on FreeBSD' do
      ...
      let(:directory) { chef_run.directory('/usr/local/etc/rsyslog.d') }
      ...
      it 'is owned by root:wheel' do
        expect(directory.owner).to eq('root')
        expect(directory.group).to eq('wheel')
      end
  ...
  context '/etc/rsyslog.conf template' do
    ...
    context 'on FreeBSD' do
      ...
      let(:template) { chef_run.template('/usr/local/etc/rsyslog.conf') }
      let(:modules) { %w(immark imuxsock imklog imtcp imudp) }
      ...
      it 'notifies restarting the service' do
        expect(template).to notify('service[rsyslogd]').to(:restart)
      end
  ...
  context '/etc/rsyslog.d/50-default.conf template' do
    ...
    context 'on FreeBSD' do
      ...
      let(:template) { chef_run.template('/usr/local/etc/rsyslog.d/50-default.conf') }
  ...
  context 'syslog service' do
    ...
    it 'stops and disables the syslog service on RHEL' do
  ...
  context 'syslogd service' do
    let(:chef_run) do
      ChefSpec::ServerRunner.new(platform: 'freebsd', version: '10.3').converge(described_recipe)
    end

    it 'stops and disables the syslog service on FreeBSD' do
      expect(chef_run).to stop_service('syslogd')
      expect(chef_run).to disable_service('syslogd')
    end

    it 'stops and disables the newsyslog service on FreeBSD' do
      expect(chef_run).to stop_service('newsyslog')
      expect(chef_run).to disable_service('newsyslog')
    end
  end
  ...
  context 'on FreeBSD' do
    ...
    it 'starts and enables the service' do
      expect(chef_run).to start_service('rsyslogd')
    end
  end
end
