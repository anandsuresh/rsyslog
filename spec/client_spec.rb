...
describe 'rsyslog::client' do
  ...
  let(:custom_remote) { [{ 'server' => '10.3.0.1', 'port' => 555, 'protocol' => 'tcp', 'logs' => 'auth.*,mail.*', 'remote_template' => 'RSYSLOG_SyslogProtocol23Format' }] }
  ...
  context '/etc/rsyslog.d/49-remote.conf template' do
    ...
    context 'on FreeBSD' do
      ...
      let(:template) { chef_run.template('/usr/local/etc/rsyslog.d/49-remote.conf') }
  ...
  context '/etc/rsyslog.d/server.conf file' do
    ...
    context 'on FreeBSD' do
      let(:chef_run) do
        ChefSpec::ServerRunner.new(platform: 'smartos', version: '10.3') do |node|
          node.set['rsyslog']['server_ip'] = server_ip
        end.converge(described_recipe)
      end

      let(:file) { chef_run.file('/usr/local/etc/rsyslog.d/server.conf') }
